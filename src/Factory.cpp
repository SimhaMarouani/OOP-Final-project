...
std::unique_ptr<StaticObjects> ObjectFactory::create(const std::string& name, sf::Vector2f pos, b2World* world, sf::Vector2f scale)
{
	...
	if (initial) {
		...
		registerit("left_floor_L1", [](sf::Vector2f pos, b2World* world, sf::Vector2f scale) -> std::unique_ptr<StaticObjects> { return std::make_unique<Floor>(pos, Grounds::Left_l1, world, scale); });
