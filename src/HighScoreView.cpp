...
bool HighScoreView::isContain(const sf::Event& event) const
{
...
void HighScoreView::setLevelScore(int level)
{
	...
	int min = 0;
	calcTime(sec, min);
	std::string scoreStr = (min < 10 ? "0" : "") + std::to_string(min) + ":" + (sec < 10 ? "0" : "") + std::to_string(sec);
...
void HighScoreView::updateData()
{
	for (int i = 0; i < m_levelsScoreText.size(); i++)
	{
		...
		else
		{
			int min = 0;
			calcTime(sec, min);
			scoreStr = (min < 10 ? "0" : "") + std::to_string(min) + ":" + (sec < 10 ? "0" : "") + std::to_string(sec);
		}
